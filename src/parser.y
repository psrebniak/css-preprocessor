...
import
    : IMPORT RAW_STRING SEMICOLON {
        std::string text = $2.toString();
        text = text.substr(1, text.length() - 2);

        driver.pushFileToQueue(text);
        std::string path = driver.getRealPath(text);
...
valueNumber
    ...
    | DOT numberHex PERCENT {
        $$ = (new CSSP::AST::Number("0." + $2.toString() + "%"))->setToken($1);
    }
    | numberHex DOT numberHex PERCENT {
        $$ = (new CSSP::AST::Number($1.toString() + "." + $3.toString() + "%"))->setToken($1);
    }
    | numberHex {
...
void CSSP::Parser::error( const location_type &l, const std::string &err_message )
{
   ...
   std::cerr << "> " << this->lexer.getCurrentLine() << std::endl;
}
